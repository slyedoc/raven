...
#[reflect(Component)]
pub struct MeshBlas(pub Handle<Blas>);

/// Bottom Level Acelleration Structure
/// An AAbb 2-ary bounding volume hierarchy (Bvh) of triangles.
/// See https://en.wikipedia.org/wiki/Bounding_volume_hierarchy
#[derive(Asset, Default, TypePath, Debug)]
pub struct Blas {
    pub nodes: Vec<BlasNode>,
    pub tris: Vec<Tri>,
    pub triangle_indexs: Vec<usize>,
}

impl From<&Mesh> for Blas {
...
impl Blas {
    pub fn new(triangles: Vec<Tri>) -> Blas {
        ...
        // Note: Due to no longer being 32 bytes, we can no longer add dummy nodes to align the tree to 64 bytes cache line
        // nodes.push(BvhNode {
        //     left_first: 0,
        //     tri_count: 0,
        //     aabb: Aabb3d::init(),
    ...
    fn subdivide_node(&mut self, node_idx: usize) {
        ...
        // create child nodes
        self.nodes.push(BlasNode::default());
    ...
    fn find_best_split_plane(&self, node: &BlasNode) -> (usize, f32, f32) {
        // determine split axis using SAH
        let mut best_axis = 0;
