...
#[relationship(relationship_target = NavTiles)]
...
pub(crate) type TileBuildResult = Option<(TileNavMesh, Aabb3d, Mesh, Blas)>;
...
pub(crate) async fn build_tile(
    ...
    let blas = Blas::from(&mesh);

    Some((nav_mesh, aabb, mesh, blas))
}
